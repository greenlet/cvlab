  ...
  for (int i = 0; i < camera_ids->numCameras; i++) {
    ...
    for (int tag_ind = 0; tag_ind < tags_n; tag_ind++) {
      if (tags[tag_ind] == ACAMERA_LENS_FACING) {
        ...
        camera_status_t
            status = ACameraMetadata_getConstEntry(camera_metadata, tags[tag_ind], &lens_info);
...
std::string CameraProps::toString(int tab_size) const {
  ...
  os << "Sensitivity: min = " << min_sensitivity << ", max = " << max_sensitivity
     << std::endl;
  os << "JPEG sizes:\n" <<
...
void CameraAndroid::setCallback(CameraCallback callback) {
...
void CameraAndroid::onDisconnected(ACameraDevice *camera_device) {
  LOG.I("Camera disconnected");
}

void CameraAndroid::onError(ACameraDevice *device, int error) {
  LOG.E("Camera error: %d", error);
}
...
//  D("Planes: %d. y_stride: %d, u_stride: %d, v_stride: %d", n_planes, y_stride, u_stride, v_stride);
...
//  D("onNewImage: format = %d. y = %d, u = %d, v = %d. y + u + v = %d. width = %d, height = %d, 1.5 * width * height = %d",
...
void CameraAndroid::start() {
  ...
  status_ = ACameraManager_openCamera(camera_manager_,
                                      camera_props_.id.c_str(),
  ...
  AImageReader_new(camera_size_.width,
                   camera_size_.height,
                   AIMAGE_FORMAT_YUV_420_888,
                   4,
                   &image_reader_);
  AImageReader_ImageListener image_callbacks{
      .context = this,
  ...
  ACameraDevice_createCaptureSession(camera_device_,
                                     capture_outputs_,
                                     &capture_session_state_callbacks,
                                     &capture_session_);
  ...
  ACameraCaptureSession_setRepeatingRequest(capture_session_,
                                            &capture_session_capture_callbacks,
                                            1,
                                            &capture_request_,
                                            nullptr);
